...
def find_linked_layer(layer, is_found):
	...
	for i in layer.layers:
		...
		# Both image and layer have this attribute, 'layers'.
		if hasattr(i, 'layers'):
...
def python_mask_from_linked(image, start_layer):
	...
	if is_found:
		...
		if pdb.gimp_item_is_valid(masqueactif) != 1:
			masque = pdb.gimp_layer_create_mask(calqueactif, ADD_MASK_SELECTION)
...
register(
	...
	"Jacques P.",
	"March 2024",
	"<Image>/Layer/Mask/Mask from linked layer",
	"RGB*, GRAY*",
	[
